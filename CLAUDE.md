...
**الإصدار:** Worker `v2.3.0` · الواجهة `v1.0.0`   ← الاتنين مستقلين، طبيعي يختلفوا
...
| `POST /webhook` | **مسار مستقل** لاستقبال `orders/create` — قبل بوابة `WORKER_SECRET` |
...
- **`SHOPIFY_WEBHOOK_SECRET` مش `CLIENT_SECRET`.** الويبهوك مسجَّل من Admin-UI، فالمفتاح هو مفتاح المتجر من Settings → Notifications → Webhooks. الخلط بينهم = `hmac_failed` متكرر.
...
آخر مطابقة: 12-09-2026 · `index.js` v2.3.0 · `index.html` v1.0.0
🔴 معلّقة: — لا شيء
